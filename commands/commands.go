...
// Get ...
func Get() Command {
	if *globals.JSONFilePath != "" || *globals.CSVFilePath != "" {
		return &getProfilesFromCSVCommand{}
	}
	return nil
}
...
var Summary = struct {
	...
	ctProcessed:   0,
	ctUnprocessed: 0, // TODO: number of rows in the csv
}
		...
		if err == nil && resp.StatusCode < 500 && !retry {
			...
			if resp.StatusCode == http.StatusOK {
				...
				if ctRespError == nil {
					...
					// nprocessed := len(respFromCT.Unprocessed)
					Summary.Lock()
					Summary.ctProcessed += 1
					Summary.ctUnprocessed -= 1
					Summary.Unlock()
				}
			}
			resp.Body.Close()
